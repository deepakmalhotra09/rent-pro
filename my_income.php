...
if(isset($_REQUEST['t_id'])){
  ...
  $e_sql = "AND tenant_id = $tenant_id";
}else if(isset($_REQUEST['p_id'])){
  $property_id = $_REQUEST['p_id'];
  $e_sql = "AND property_id = $property_id";
}
$sql = "SELECT * FROM rentre_invoice WHERE user_id = '$user_id' AND status IN ('1', '2') $e_sql";
$invoices = $conn->query($sql);
$results = array('pending' => array('total' => 0, 'records' => array()) ,'paid' => array('total' => 0, 'records' => array()));
while($row = $invoices->fetch_assoc()){
  ...
  $results[$stats]['total'] += $row['total_amount'];
  $results[$stats]['records'][] = $row;
}
// print_r($results);
// die;
?>
        ...
        <div class="right_col" role="main">
          ...
          <div class="row">
						<div class="col-md-12 col-sm-12 ">
							<div class="x_panel">
								<div class="x_title">
									<h2>Total Received Amount: <i class="fa fa-rupee"></i> <?= number_format($results['paid']['total']); ?>
                      ...
                      <div class="col-sm-12">
                        <div class="card-box table-responsive">
                          <table id="datatable" class="table table-striped table-bordered" style="width:100%">
                            ...
                            <tbody>
                              <?php foreach ($results['paid']['records'] as $key => $row){
                                $t_id = $row['tenant_id'];
            ...
            <div class="row">
  						<div class="col-md-12 col-sm-12 ">
  							<div class="x_panel">
  								<div class="x_title">
  									<h2>Total Pending Amount: <i class="fa fa-rupee"></i> <?= number_format($results['pending']['total']); ?>
                      <!-- <small>different form elements</small> -->
                    </h2>
  									<ul class="nav navbar-right panel_toolbox">
  										<li><a class="collapse-link"><i class="fa fa-chevron-up"></i></a>
                        ...
                        <div class="col-sm-12">
                          <div class="card-box table-responsive">
                            <table id="datatable-buttons" class="table table-striped table-bordered" style="width:100%">
                              ...
                              <tbody>
                                <?php foreach ($results['pending']['records'] as $key => $row){
                                  $t_id = $row['tenant_id'];
                                  $sql_t = "SELECT * FROM rentre_tenants WHERE id = $t_id";
                                  $tenant_sql = $conn->query($sql_t);
                                  $tenant = $tenant_sql->fetch_object();
